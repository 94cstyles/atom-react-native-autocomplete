...
var collect = {
    ...
    parseMethod: function parseMethod(node, url) {
    ...
    parsePropertie: function parsePropertie(node, url) {
    ...
    parseProp: function parseProp(node, url) {
    ...
    reloadPage: function reloadPage(type, name, url) {
        ...
        (0, _request2.default)(url, function (error, response, body) {
            ...
            _this2.outputFile(type, _this2.getSubClass(type, name, url, $));
        });
    },
    find: function find() {
        var _this3 = this;
            ...
            for (var i = 0; i < menuNode.length; i++) {
                ...
                if (title.match(/apis|components/gi)) {
                    ...
                    for (var j = 0; j < subMenuNode.length; j++) {
                        _this3.reloadPage(title, subMenuNode.eq(j).html(), 'http://facebook.github.io' + subMenuNode.eq(j).attr('href'));
                    }
                }
            }
        });
